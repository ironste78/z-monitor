...
public class Provider extends Classes.ZGUIImpl
{
   ...
   //**********************CONSTRUACTOR********************************//
	...
	public void insertInDatabase ()
	{
		...
		String receivedPacketStream = ZM.packetBuffer.getOrigonalPackets().get(j);
                Double packetTimeStampDB= ZM.packetBuffer.getOrigonalTimes().get(j);
		if(ZM.connection.isConected())
		{
			//If Z-Server is enabled store the packet
			ZM.connection.insertData(receivedPacketStream, ZM.connection.getIdSnifer(),packetTimeStampDB, ZM.connection.getTask());
		}
	}
    }
        ...
        public void changeConfigZCode()
	{
		...
		final JPasswordField passtextField = new JPasswordField(15);
                ...
                final JLabel snifferidLabel = new JLabel("Sniffer ID:");
		final JLabel taskLabel = new JLabel("If the task exists, insert the ID, else a new one will be create:");
		final JTextField snifferidtextField = new JTextField((UUID.randomUUID()).toString());
		snifferidtextField.setEnabled(false);
        ...
        public void createAndShowGUI()
	{
		...
		// Create a buttons //
        	...
        	dbButton.setToolTipText("Change DB ZServer configuration");portButton.setToolTipText("Change Port F4");
